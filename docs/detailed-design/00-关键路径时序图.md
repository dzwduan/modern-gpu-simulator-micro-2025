...
flowchart LR
    subgraph FRONT["Frontend"]
        direction LR
        F["Cycle N<br/>fetch<br/>L0I -> 取指缓冲"]
        D["Cycle N+1<br/>decode<br/>填充 IBuffer entry"]
        I["Cycle N+2<br/>issue<br/>stall / yield / barrier / FU / 结果队列检查"]
        F -->|"ifetch_buf"| D
        D -->|"IBuffer entry 有效"| I
    end

    subgraph DISPATCH["Dispatch To FU"]
        direction LR
        C["Cycle N+3<br/>control<br/>登记 barrier increment 到 SM pending stack"]
        A["Cycle N+4<br/>allocate<br/>预留 RF 读端口与 FU latency 槽位"]
        R["Cycle N+5~N+7<br/>read_rf pipeline<br/>推进 read-stage latency 寄存器"]
        C -->|"m_CONTROL_ALLOCATE_latch"| A
        A -->|"m_pipeline_read_stage_latency_reg[latency-1]"| R
    end

    subgraph EXECUTE["Execution And Completion"]
        direction LR
        E["Cycle N+7+<br/>execute<br/>fu->issue() -> m_pipeline_reg[]"]
        W["Cycle N+8+<br/>writeback<br/>弹出 fixed-latency rf_write_queue"]
        RT["retire<br/>SM::instruction_retirement()<br/>登记 write-barrier decrement"]
        E --> W --> RT
    end

    I -->|"ISSUE_CONTROL_latch"| C
    R -.->|"尾级调用 release_read_barrier()"| E
```
...
```mermaid
flowchart LR
    FETCH[Subcore fetch decode issue]
    CTRL[Control variable latency]
    MEMQ[Memory FU queue]
    RX[Shared reception latch]
    ARB[Ldst reception arb]
    ASSIGN[PRT assign]
    SELECT[PRT select]
    COAL[Access gen coalesce]
    ROUTE[Route by memory space]
    GLB[Global Local L1D]
    SHM[Shared Shmem]
    CON[Const L1C]
    TEX[Texture L1T]
    BYP[Bypass L2 ICNT]
    DONE[Pending access solve]
    POP[PRT pop]
    WB[Writeback latch]
    RET[RF writeback retire]

    FETCH --> CTRL --> MEMQ --> RX --> ARB --> ASSIGN --> SELECT --> COAL --> ROUTE
    ROUTE --> GLB
    ROUTE --> SHM
    ROUTE --> CON
    ROUTE --> TEX
