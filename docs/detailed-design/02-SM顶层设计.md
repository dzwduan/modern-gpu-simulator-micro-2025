...
- 显式加入论文强调的 `Control` 与 `Allocate` 两级（位于 `Issue` 与 `RF Reads` 之间），用于区分固定延迟和可变延迟路径的资源检查语义。
...
flowchart TB
    ...
    subgraph LDSTU["共享访存单元 (ldst_unit_sm)"]
        REC["Reception ports\nNUM_SUBCORES + 1 输入"]
