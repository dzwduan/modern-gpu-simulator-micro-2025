...
```
传输成立条件：valid=1 && ready=1（在同一周期/事务边界）
背压行为：    ready=0 时，发送端必须保持 valid 和 payload 稳定，直到 ready 拉高
顺序保证：    同一端口上的传输严格按发送顺序到达（FIFO 语义）
```
...
```
1. [Spec] 模块概述
   - 一段话说明功能、在 GPU 微架构中的位置与职责边界

2. [Spec] 参数定义
   - 参数名、含义、取值范围、对位宽/资源规模的影响

3. [Spec] 数据类型
   - 模块特有的 payload 类型（如果有）；通用类型引用 §0A.2

4. [Spec] 外部接口
   - Input/Output 端口表，按 §0A.3 列定义
   - 每个端口标注 CA/TL/FN 粒度差异
...
7. [Spec] 多粒度行为差异
   - CA/TL/FN 三种粒度下的行为裁剪规则

8. [Spec] 设计原理
   - 为什么这样设计（不只是"是什么"）
   - 与真实 GPU 的对应关系（只讲可验证共性）

9. [Map] framework 映射
   - 每个外部接口的 framework 落地方式
   - 背压与恢复路径
   - framework 锚点示例

10. [Map] 源码锚点
    - 微架构源码锚点（主锚点）
    - framework 锚点（实现参考锚点，单独标注）
```
