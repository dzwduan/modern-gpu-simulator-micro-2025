...
> **配置矩阵澄清**（见 `docs/plans/2026-07-15-remodeled-trace-p0-p1-semantic-repair.md` §3，权威来源）：
> 下面两张表中的 `is_interwarp_coalescing_enabled`、`is_instruction_prefetching_enabled`、
> `is_fp32_and_int_unified_pipeline` 三项并非固定不变量，而是 remodeled-trace 产品能力矩阵中
> 的可配置支持项（分别支持 on/off、on/off、unified/separate，interwarp coalescing 还与 PRT
> 选择策略联动）。本章列出的 `true` 是论文对齐讨论所使用的参考取值，不代表运行时唯一支持值；
> 其余各项（含 `is_remodeling_scoreboarding_enabled=false`）仍是固定不变量。
...
| `is_remodeling_scoreboarding_enabled` | `false` | **唯一例外**。禁用传统 scoreboard，issue/retire 不调用 Scoreboard::checkCollision/releaseRegisters | ⚠️ Section 7.5 |
...
| `is_remodeling_scoreboarding_enabled` | `bool` | 1 | `0` | 0=禁用传统 scoreboard, 1=启用 | Subcore issue/SM retire（依赖检测路径） | ⚠️ |
...
> 总计 13 个布尔配置位，本章论文对齐讨论下 12 个取参考值 `true`，仅 `is_remodeling_scoreboarding_enabled`
> 固定为 `false`；其中 3 个（见上方配置矩阵澄清）是可配置支持矩阵参数，而非固定不变量。
>
> **论文验证状态**: ✅ 完全验证 (7个) | ⚠️ 需澄清 (4个) | ❌ 未验证 (2个)
...
| 假设 | 冲突/问题 | 论文证据 | 建议 |
|------|-----------|----------|------|
| `is_remodeling_scoreboarding_enabled=false` | 论文证明 control bits 优于 scoreboard，但未明确说硬件中完全不存在 scoreboard | Section 7.5 Table 7 对比了 scoreboard 和 control bits 的性能 | 确认该值表示"True-Path 下不启用"而非"硬件中不存在" |
