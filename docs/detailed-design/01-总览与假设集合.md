...
**注意**: 部分假设（如 INT/FP32 pipeline 统一性 `is_fp32_and_int_unified_pipeline`）可能存在架构代际差异。论文 Figure 3 显示 INT32 和 FP32 为独立 execution unit，但代码中两者被配置为统一（INT 指令走 SP pipeline）。两种配置均在代码中得到支持，使用时需注意目标架构的实际硬件实现。
...
| 参数 | 值 | 实现影响 | 论文验证 |
|---|---|---|---|
| `is_trace_mode` | `true` | 指令来源于 trace 文件，`get_next_trace_inst(pc)` 获取指令 | ✅ Section 3 |
| `is_captured_from_binary` | `true` | control bits 有效，依赖走 control-bit 路径 | ✅ Section 4 |
| `is_SM_remodeling_enabled` | `true` | 入口选择 `SM` 而非 legacy shader_core_ctx | ✅ Section 6 |
| `is_remodeling_scoreboarding_enabled` | `false` | **唯一例外**。传统 scoreboard 代码路径已在重构中移除，仅保留 control-bit 依赖管理 | ✅ Section 7.5 |
| `is_ibuffer_remodeled_enabled` | `true` | 使用 `IBuffer_Remodeled`（deque 结构），不回退旧 IBuffer | ✅ Figure 3 |
| `is_interwarp_coalescing_enabled` | `true` | 访存经 `InterWarpCoalescingUnit` 合并后再进入子流水线 | ✅ Section 5.4 |
| `is_instruction_prefetching_enabled` | `true` | L0I stream buffer 预取开启 | ✅ Section 7.3 |
| `is_fp32_and_int_unified_pipeline` | `true` | INT/PREDICATE 指令走 SP pipeline，不创建独立 INT FU | ⚠️ Figure 3 |
| `is_fp32ops_allowed_in_int_pipeline` | `true` | 该项仅在存在独立 INT pipeline 时有意义；unified 配置下应视为无效并避免触发 INT pipeline 路径 | ⚠️ 依赖上项 |
| `is_dp_pipeline_shared_for_subcores` | `true` | DP 指令经 subcore 的 `m_dp_pipeline` 送入 SM 共享 `m_shared_dp_unit` | ⚠️ 论文未详述 |
| `is_rf_cache_enabled` | `true` | regular RF 使用 cacheable 读路径 `is_possible_to_read_cacheable()` | ✅ Section 5.3.1, 7.4 |
| `is_loog_enabled` | `true` | pending writes first key 使用 `m_cu_rrs_id`（Load Operand Ordering Graph） | ❌ 实现细节 |
| `is_vpreg_enabled` | `true` | pending writes second key 使用 `vpreg_virtual_out[idx]`（Virtual Physical Register） | ❌ 实现细节 |

### 1.3.1 True-Path 假设矩阵存储结构

以下表格将 True-Path 假设集合形式化为位宽表示，描述每个配置位在硬件/软件中的存储形式：

| 配置参数 | 存储类型 | 位宽 | True-Path 值 | 编码 | 影响的模块 | 论文验证 |
|---|---|---|---|---|---|---|
...
| `is_remodeling_scoreboarding_enabled` | `bool` | 1 | `0` | 0=control-bit 路径（传统 scoreboard 代码已移除）, 1=已废弃 | 历史配置，代码已重构移除 scoreboard 分支 | ✅ |
...
| `is_fp32_and_int_unified_pipeline` | `bool` | 1 | `1` | 0=独立 INT FU, 1=INT/PRED 共享 SP pipeline | Subcore issue（FU 选择逻辑） | ⚠️ |
| `is_fp32ops_allowed_in_int_pipeline` | `bool` | 1 | `1` | unified 模式下无效 | Subcore issue | ⚠️ |
| `is_dp_pipeline_shared_for_subcores` | `bool` | 1 | `1` | 0=subcore 私有 DP, 1=SM 共享 DP | Subcore→SM shared DP unit | ⚠️ |
...
>
> **论文验证状态**: ✅ 完全验证 (8个，含重构后已解决的 scoreboard 项) | ⚠️ 需澄清 (3个) | ❌ 未验证 (2个)

### 1.3.2 论文验证状态说明

**验证等级定义**:

- ✅ **完全验证**: 论文明确描述并通过实验验证该特性，章节引用可追溯
- ⚠️ **需澄清**: 论文部分支持但存在细节冲突或未提供足够实现细节
- ❌ **未验证**: 论文未提及，属于实现细节或基于其他来源推断

**需澄清假设的详细说明**:

| 假设 | 冲突/问题 | 论文证据 | 建议 |
|------|-----------|----------|------|
| `is_remodeling_scoreboarding_enabled=false` | 论文证明 control bits 优于 scoreboard（Section 7.5 Table 7）。重构已移除传统 scoreboard 代码路径，不再可配置 | Section 7.5 Table 7 | 已解决：scoreboard 代码路径已移除 |
| `is_fp32_and_int_unified_pipeline=true` | 论文 Figure 3 显示 INT32 和 FP32 是独立的 execution units | Figure 3 SM/Core 架构图 | 查阅 NVIDIA 官方白皮书确认物理实现 |
| `is_fp32ops_allowed_in_int_pipeline=true` | 依赖上一项的澄清 | 论文未提及 | 与上一项联动处理 |
| `is_dp_pipeline_shared_for_subcores=true` | 论文未提供 DP 单元拓扑的足够细节 | Figure 3 显示 FP64 但未标注共享关系 | 通过微基准测试验证或查阅官方文档 |

**未验证假设的说明**:

- `is_loog_enabled` 和 `is_vpreg_enabled`: 这两个参数控制访存单元中 pending writes 的排序键选择，属于实现细节。论文聚焦于高层架构（control bits、RF cache、instruction prefetching），未涉及 pending writes 的内部排序机制。这些特性可能基于源码分析或其他文献推断。

**论文章节映射**:

| 论文章节 | 对应假设 | 关键内容 |
|----------|----------|----------|
| Section 3 | `is_trace_mode` | Trace-driven 方法论，通过微基准测试提取 SASS 指令 |
| Section 4 | `is_captured_from_binary` | Control bits 语义（stall counter、yield、wait barriers） |
| Section 5.3.1, 7.4 | `is_rf_cache_enabled` | Register File Cache (RFC) 设计与性能影响 |
| Section 5.4 | `is_interwarp_coalescing_enabled` | Inter-warp memory coalescing |
| Section 6 | `is_SM_remodeling_enabled` | SM/core 模型从头设计 |
| Section 7.3, Table 5 | `is_instruction_prefetching_enabled` | Stream buffer 预取与性能评估 |
| Section 7.5, Table 7 | `is_remodeling_scoreboarding_enabled` | Control bits vs Scoreboard 性能对比 |
| Figure 3 | `is_ibuffer_remodeled_enabled`, `is_fp32_and_int_unified_pipeline` | SM/Core 架构图 |

---

## 1.4 Control-Bit 依赖模型（唯一路径）

重构后，传统 scoreboard 代码路径已被移除，仅保留 control-bit 依赖管理：

- **issue 阶段**：就绪判定直接走 `Dependency_State`（stall/yield/wait barrier），无 scoreboard 检查
- **control 阶段**：直接识别 control bits 中的 read/write barrier 并产生 pending increment
- **retire 阶段**：直接产生 write barrier pending decrement
- **mem barrier**：直接检查 `are_all_wait_barrier_ready()`
- **read barrier**：FU 完成时直接产生 read barrier pending decrement

> 历史说明：重构前代码中存在 6 处 guard condition，使用 `use_traditional_scoreboarding` 变量在 scoreboard 和 control-bit 两条路径间切换。重构移除了所有 scoreboard 分支，简化了 `subcore.cc`（issue/control_stage）、`sm.cc`（instruction_retirement/issue_warp/warp_waiting_at_mem_barrier）和 `functional_unit.cc`（release_read_barrier）中的代码路径。
...
```mermaid
stateDiagram-v2
    IDLE: IDLE<br/>(stall=0, yield=0,<br/>barriers ready, ldgsts=0)
    STALLED: STALLED<br/>(stall_counter = N)
    YIELDED: YIELDED<br/>(m_yield = 2)
    BARRIER_WAIT: BARRIER_WAIT<br/>(wait_barriers[id].counter > 0)
    LDGSTS_PENDING: LDGSTS_PENDING<br/>(m_num_pending_ldgsts > 0)
