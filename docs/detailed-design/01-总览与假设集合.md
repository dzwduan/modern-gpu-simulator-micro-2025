...
stateDiagram-v2
    ...
    IDLE --> STALLED: issue 阶段设置 stall_count\nstall_counter = N (从 control bits 提取)
    STALLED --> IDLE: stall_counter == 0\n(每周期: stall_counter >>= 1)

    IDLE --> YIELDED: issue 阶段设置 yield\nm_yield = 2
    YIELDED --> IDLE: m_yield == 0\n(每周期: m_yield >>= 1)

    IDLE --> BARRIER_WAIT: control 阶段 barrier increment\nwait_barriers[id].increase_counter()
    BARRIER_WAIT --> IDLE: issue 阶段检查 wait_barrier_bits\n且所有 barrier ready

    IDLE --> LDGSTS_PENDING: LDGSTS 发射\nm_num_pending_ldgsts++
    LDGSTS_PENDING --> IDLE: m_num_pending_ldgsts == 0\n(LDGSTS 完成: m_num_pending_ldgsts--)
```
