...
// At file scope, declare the texture objects
cudaTextureObject_t t_features;
cudaTextureObject_t t_features_flipped;
cudaTextureObject_t t_clusters;

// Create texture objects during initialization
void createTextureObjects(float* features_d, float* features_flipped_d, float* clusters_d, 
                         size_t features_size, size_t features_flipped_size, size_t clusters_size) {
    ...
    // Create t_features texture
    resDesc.res.linear.devPtr = features_d;
    resDesc.res.linear.desc = cudaCreateChannelDesc<float>();
    resDesc.res.linear.sizeInBytes = features_size * sizeof(float);
    cudaCreateTextureObject(&t_features, &resDesc, &texDesc, NULL);
    
    // Create t_features_flipped texture
    resDesc.res.linear.devPtr = features_flipped_d;
    resDesc.res.linear.sizeInBytes = features_flipped_size * sizeof(float);
    cudaCreateTextureObject(&t_features_flipped, &resDesc, &texDesc, NULL);
    
    // Create t_clusters texture
    resDesc.res.linear.devPtr = clusters_d;
    resDesc.res.linear.sizeInBytes = clusters_size * sizeof(float);
...
kmeansPoint(float  *features,			/* in: [npoints*nfeatures] */
			...
			int    *block_deltas,
            cudaTextureObject_t tex_features,     /* texture object for features */
            cudaTextureObject_t tex_features_flipped, /* texture object for flipped features */
            cudaTextureObject_t tex_clusters)     /* texture object for clusters */
{
	...
	if (point_id < npoints)
	{
		...
		for (i=0; i<nclusters; i++) {
			...
			{					
				...
				float diff = tex1Dfetch<float>(tex_features, addr);	/* distance between a data point to cluster centers */
	...
	***/ 	
	...
	if(threadIdx.x < nfeatures * nclusters) {
		...
		for(int i = 0; i< (THREADS_PER_BLOCK); i++) {
			float val = tex1Dfetch<float>(tex_features_flipped, new_base_index+i*nfeatures);
