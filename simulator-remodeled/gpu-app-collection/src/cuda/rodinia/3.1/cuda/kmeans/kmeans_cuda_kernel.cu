...
extern cudaTextureObject_t t_features_obj;
// t_features_flipped has the layout point0[dim 0-n-1]point1[dim 0-n-1]
extern cudaTextureObject_t t_features_flipped_obj;
extern cudaTextureObject_t t_clusters_obj;
...
kmeansPoint(float  *features,			/* in: [npoints*nfeatures] */
			...
			int    *block_deltas,
            cudaTextureObject_t t_features_tex,
            cudaTextureObject_t t_features_flipped_tex) 
{
	...
	if (point_id < npoints)
	{
		...
		for (i=0; i<nclusters; i++) {
			...
			{					
				...
				float diff = (tex1Dfetch<float>(t_features_tex, addr) -
	...
	***/ 	
	...
	if(threadIdx.x < nfeatures * nclusters) {
		...
		for(int i = 0; i< (THREADS_PER_BLOCK); i++) {
			float val = tex1Dfetch(t_features_flipped_tex,new_base_index+i*nfeatures);
