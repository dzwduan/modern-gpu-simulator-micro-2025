...
__global__ void
mergeSortFirst(float4 *result, int listsize, cudaTextureObject_t tex_obj) 
{
    // Block index
    int bx = blockIdx.x;
    // Thread index
    ...
    if(bx*blockDim.x + threadIdx.x < listsize/4){
        float4 r = tex1Dfetch<float4>(tex_obj, (int)(bx*blockDim.x + threadIdx.x));
        result[bx * blockDim.x + threadIdx.x] = sortElem(r); 
    }
}

__global__ void
mergeSortPass(float4 *result, int nrElems, int threadsPerDiv, cudaTextureObject_t tex_obj) 
{
    ...
    if(Bstart >= constStartAddr[division + 1]){
        ...
        {
            resStart[i] = tex1Dfetch<float4>(tex_obj, Astart + i); 
        }
        return; 
    }
    ...
    int outidx = 0; 
    float4 a, b;
    a = tex1Dfetch<float4>(tex_obj, Astart + aidx);  
    b = tex1Dfetch<float4>(tex_obj, Bstart + bidx); 
    ...
    while(true)
    {
         ...
         * after the merge
         */
        float4 nextA = tex1Dfetch<float4>(tex_obj, Astart + aidx + 1); 
        float4 nextB = tex1Dfetch<float4>(tex_obj, Bstart + bidx + 1); 
