...
extern cudaTextureObject_t texPivot; // Add external declaration for the texture object
cudaTextureObject_t texPivot = 0; // Define the texture object globally
...
void finish_bucketsort()
{
	...
	// Destroy texture object if it exists
	if (texPivot) {
				...
				unsigned int *origOffsets)
{
	...
	///////////////////////////////////////////////////////////////////////////
	checkCudaErrors(cudaMemcpy(l_pivotpoints, pivotPoints, (DIVISIONS)*sizeof(float), cudaMemcpyHostToDevice)); 
    ...
    dim3 grid(blocks, 1);
	// Find the new indice for all elements - Pass texture object as parameter
