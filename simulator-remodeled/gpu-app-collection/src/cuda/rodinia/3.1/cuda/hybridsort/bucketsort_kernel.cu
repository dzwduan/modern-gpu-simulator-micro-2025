...
// Remove the local definition and make it a kernel parameter instead
...
__global__ void
bucketcount( float *input, int *indice, unsigned int *d_prefixoffsets, int size, cudaTextureObject_t texPivot)
{
	...
	for (int tid = blockIdx.x * blockDim.x + threadIdx.x; tid < size; tid += numThreads) {
		...
		// Update the texture fetch with passed texture object
		float piv = tex1D<float>(texPivot, idx);

		while(jump >= 1){
			idx = (elem < piv) ? (idx - jump) : (idx + jump);
			// Update the texture fetch with passed texture object
			piv = tex1D<float>(texPivot, idx);
			jump /= 2; 
		}
