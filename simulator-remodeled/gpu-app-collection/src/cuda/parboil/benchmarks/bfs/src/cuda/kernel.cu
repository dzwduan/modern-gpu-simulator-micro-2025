...
#include "config.h"
// Add __device__ qualifier to texture objects to make them accessible in device code
__device__ cudaTextureObject_t g_graph_node_ref;
__device__ cudaTextureObject_t g_graph_edge_ref;
	   ...
	   int gray_shade)
{
  ...
  // Fix texture fetch - use the correct form for CUDA 12
  Node cur_node = tex1Dfetch<Node>(g_graph_node_ref, pid);

  // For each outgoing edge
  for(int i = cur_node.x; i < cur_node.y + cur_node.x; i++) {
    // Fix texture fetch for edge
