...
int main(int argc, char** argv) 
{
  ...
  //Create texture objects instead of binding textures
  cudaResourceDesc resDesc;
  memset(&resDesc, 0, sizeof(resDesc));
  resDesc.resType = cudaResourceTypeLinear;
  
  // Create texture object for graph nodes
  resDesc.res.linear.devPtr = d_graph_nodes;
  resDesc.res.linear.desc.f = cudaChannelFormatKindSigned;
  resDesc.res.linear.desc.x = sizeof(int) * 8; // bits per channel
  resDesc.res.linear.desc.y = sizeof(int) * 8;
  resDesc.res.linear.desc.z = 0;
  resDesc.res.linear.desc.w = 0;
  resDesc.res.linear.sizeInBytes = sizeof(Node) * num_of_nodes;
  
  cudaTextureDesc texDesc;
  memset(&texDesc, 0, sizeof(texDesc));
  texDesc.readMode = cudaReadModeElementType;
  
  cudaCreateTextureObject(&g_graph_node_ref, &resDesc, &texDesc, NULL);
  
  // Create texture object for graph edges
  resDesc.res.linear.devPtr = d_graph_edges;
  resDesc.res.linear.sizeInBytes = sizeof(Edge) * num_of_edges;
  
  cudaCreateTextureObject(&g_graph_edge_ref, &resDesc, &texDesc, NULL);
  ...
  // Destroy texture objects instead of unbinding textures
