...
// Add new kernel that uses texture object
__global__ void spmv_jds_tex(float *dst_vector,
						const float *d_data,const int *d_index, const int *d_perm,
						cudaTextureObject_t x_tex,const int *d_nzcnt,const int dim)
{
	int ix=blockIdx.x*blockDim.x+threadIdx.x;
	int warp_id=ix>>WARP_BITS;
	if(ix<dim)
	{
		...
		float d = d_data[j]; 
		int i = d_index[j];  
		float t = tex1Dfetch<float>(x_tex, i);
