...
int main(int argc, char** argv) {
  ...
  input_vec( parameters->inpFiles[1],h_x_vector,dim);
	...
	// Create texture object for x_vector
	...
	cudaCreateTextureObject(&x_tex, &resDesc, &texDesc, NULL);
  ...
  cudaFuncSetCacheConfig(spmv_jds, cudaFuncCachePreferL1);
	...
	for (int i= 0; i<50; i++)
	{
		// Use the texture-based kernel if CUDA version >= 12, otherwise use original
		#if __CUDACC_VER_MAJOR__ >= 12
		spmv_jds_tex<<<grid, block>>>(d_Ax_vector,
	...
	if (x_tex) {
		cudaDestroyTextureObject(x_tex);
	}
