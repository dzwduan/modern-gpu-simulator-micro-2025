                ...
                cudaTextureObject_t refTex)
{
  ...
  if ((mb_x < mb_width) && (mb_y < mb_height))
	...
	for(int y=0; y<4; y++) {
	  int t;
	  t = tex2D<unsigned short>(refTex, search_off_x, search_off_y + y);
	  sad1 += abs(t - FRAME_GET(cur_o, 0, y));

	  t = tex2D<unsigned short>(refTex, search_off_x + 1, search_off_y + y);
	  sad1 += abs(t - FRAME_GET(cur_o, 1, y));
	  sad2 += abs(t - FRAME_GET(cur_o, 0, y));

	  t = tex2D<unsigned short>(refTex, search_off_x + 2, search_off_y + y);
	  sad1 += abs(t - FRAME_GET(cur_o, 2, y));
	  sad2 += abs(t - FRAME_GET(cur_o, 1, y));
	  sad3 += abs(t - FRAME_GET(cur_o, 0, y));

	  t = tex2D<unsigned short>(refTex, search_off_x + 3, search_off_y + y);
	  sad1 += abs(t - FRAME_GET(cur_o, 3, y));
	  sad2 += abs(t - FRAME_GET(cur_o, 2, y));
	  sad3 += abs(t - FRAME_GET(cur_o, 1, y));

	  t = tex2D<unsigned short>(refTex, search_off_x + 4, search_off_y + y);
	  sad2 += abs(t - FRAME_GET(cur_o, 3, y));
	  sad3 += abs(t - FRAME_GET(cur_o, 2, y));

	  t = tex2D<unsigned short>(refTex, search_off_x + 5, search_off_y + y);
...
/* Create a texture object for the reference texture */
cudaTextureObject_t create_ref_texture(cudaArray* ref_array) {
