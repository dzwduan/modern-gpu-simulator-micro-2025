...
__host__ void buildHashMap(thrust::device_vector<uint>& key, thrust::device_vector<uint>& val,const uint size) {
  ...
  // Use explicit namespace qualification for all Thrust functions
  thrust::inclusive_scan(
     thrust::make_transform_iterator(
        thrust::make_zip_iterator(thrust::make_tuple(
          thrust::make_transform_iterator(
              thrust::make_zip_iterator(thrust::make_tuple(key.begin() + 1, key.begin())), 
              neqAdapter()), 
     ...
     thrust::make_transform_iterator(
         thrust::make_zip_iterator(thrust::make_tuple(
             thrust::make_transform_iterator(
                 thrust::make_zip_iterator(thrust::make_tuple(key.begin() + size, key.begin() + size - 1)), 
                 neqAdapter()), 
          thrust::counting_iterator<uint>(1))), 
         mulAdapter()), 
     key.begin() + 1, uintMax);
