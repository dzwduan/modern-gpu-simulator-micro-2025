...
void ELFFile::write(std::ostream& out)
{
	...
	{
		unsigned int type = sectionHeader(s).header().sh_type;
...
std::string ELFFile::sectionTypeToString(unsigned int type)
{
	switch(type)
	{
...
std::string ELFFile::sectionLinkToString(unsigned int type)
{
	switch(type)
	{
...
bool ELFFile::sectionTypeHasLink(unsigned int type)
{
