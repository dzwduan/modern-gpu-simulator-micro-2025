...
public:
	...
	static std::string sectionTypeToString(unsigned int type);
	/*! \brief Does this section type have a link? */
	static bool sectionTypeHasLink(unsigned int type);
	/*! \brief Get a string representation of a section linkage */
	static std::string sectionLinkToString(unsigned int type);
