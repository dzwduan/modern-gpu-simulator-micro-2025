...
* Traces format:
  ...
  ```bash
  #traces format = threadblock_x threadblock_y threadblock_z warpid_tb PC mask unique_function_id num_mem_refs [mem_width] [adrrescompress?] [mem_addresses]
  ```
  The other columns that are in brackets [] may or may not exist based on the instruction characteristics, for example:

  Finally, the memory instructions rule is as following:
  If num_mem_refs=0, this implies that it is not a memory instruction and [mem_width] [adrrescompress?] [mem_addresses] will be empty.
  If num_mem_refs>0, this implies that this is a memory instruction with mem_width as the memory width of the data to be loaded per thread, and [adrrescompress?] [mem_addresses] will list the memory addresses in a compressed format.

  In the case of `CALL` or `RET` insturctions it encodes a num_mem_refs = 1 and mem_width= 1. The encoded address is the PC target address of the instructions.

  Example:

  31 0 0 3 0000 ffffffff 3 0
